...
const PopupDetail = ({ memberId }) => {
  return (
    <div className="absolute right-6 bg-[#274698] rounded-2xl py-4 px-4 text-[#1a1a1a] flex flex-col gap-3">
        <Link href={`${memberId}`} className="flex gap-3 items-center justify-start bg-[#fff] rounded-xl py-2 px-4 text-[#1a1a1a]">
            <TbEye className="text-[20px] text-[#666]" />
            <p className="text-[16px] font-medium">Lihat</p>
        </Link>
        <Link href={`${memberId}/edit`} className="flex gap-3 items-center justify-start bg-[#fff] rounded-xl py-2 px-4 text-[#1a1a1a]">
