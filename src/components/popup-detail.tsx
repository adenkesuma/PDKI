...
const PopupDetail = ({ npaPdki } : Props) => {
  return (
    <div className="absolute right-6 bg-[#fff] rounded-2xl shadow-md shadow-gray-400 py-4 px-4 text-[#1a1a1a] flex flex-col gap-3">
        ...
        <Link href={`member/${npaPdki}/edit`} className="flex gap-3 items-center justify-start bg-[#fff] hover:bg-gray-100 duration-75 rounded-xl py-2 px-4 text-[#1a1a1a]">
            <TbEdit className="text-[20px]" />
            <p className="text-[16px] font-medium">Edit</p>
        </Link>
        <button className="flex gap-3 items-center justify-start bg-[#fff] hover:bg-red-600 text-red-600 hover:text-[#fff] duration-75 rounded-xl py-2 px-4">
