    ...
    const dataFomc = data.map((item : MemberProps) => {
        if( item?.subspesialisasi === "Family Oriented Medical Care (FOMC)" ){
            return (
                <div 
                    key={item.npa_pdki}
                    className='bg-[#fff] shadow-md shadow-gray-300 p-4 rounded-2xl flex flex-col gap-6 lg:flex-row'
                >
                    <figure className='w-[150px] h-[220px] mx-auto md:mx-0'>
                        <Image 
                            className="w-[150px] h-[220px] object-cover rounded-xl"
                    ...
                    <div className='mt-4'>
                        <p className='mb-2 font-medium text-[16px] text-center md:text-left'>Nama: {item?.nama_sertifikat}</p>
